"""1m Momentum Surge — right-side trend entry.

Entry: when 1H price move exceeds threshold AND 4H trend aligns,
enter in the direction of the move. Pure right-side momentum.

The idea: significant hourly moves tend to continue for several more hours.
We ride the wave, not predict it.

Stop: 0.5% (tight, since we enter during strong momentum).
Trail: 2%+ activation, 1% distance.
"""
...
from strategy.indicators import ema_update
...
DEFAULT_PARAMS = {
    ...
    "allowed_regimes": ["strong_trend", "weak_trend", "breakout"],
}


@auto_register("scalp_momentum")
    ...
    async def on_bar(self, symbol: str, bar: dict[str, Any]) -> list[Signal]:
        ...
        if pos is None:
            ...
            if surge_pct > surge_threshold and e4hf > e4hs:
                ...
                if close_ratio >= self.get_param("min_close_ratio"):
                    regime_info = self._regime[symbol].current_regime.value
                    signals.append(Signal(
                        strategy_id=self.strategy_id, symbol=symbol,
                        signal_type=SignalType.LONG_ENTRY, strength=0.9, price=c,
                        reason=f"SURGE BUY {surge_pct:.2f}% [{regime_info}]",
                ...
                if close_ratio >= self.get_param("min_close_ratio"):
                    ...
                    signals.append(Signal(
                        strategy_id=self.strategy_id, symbol=symbol,
                        signal_type=SignalType.SHORT_ENTRY, strength=0.9, price=c,
                        reason=f"SURGE SELL {surge_pct:.2f}% [{regime_info}]",
                        metadata={"regime": regime_info, "atr": atr},
                    ))
                    self._hb[symbol] = 0
                    self._peak[symbol] = l
